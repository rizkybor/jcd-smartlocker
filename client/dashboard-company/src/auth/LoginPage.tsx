    ...
    >
      <div style={{ width: '100%', maxWidth: 380 }}>
        <div style={{ textAlign: 'center', marginBottom: 'var(--sl-space-6)' }}>
          ...
          <div style={{ marginTop: 4, fontSize: 'var(--sl-fs-13)', color: 'var(--sl-text-muted)' }}>{t('loginPage.subjudul')}</div>
        </div>
        <Panel>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sl-space-4)' }}>
            <Field label={t('loginPage.email')} type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t('loginPage.emailPlaceholder')} />
            <Field label={t('loginPage.password')} type="password" required value={password} onChange={(e) => setPassword(e.target.value)} />
            ...
            <Button type="submit" tone="primary" fullWidth disabled={submitting || !email || !password}>
              {submitting ? t('loginPage.masukLoading') : t('loginPage.masuk')}
            </Button>
          </form>
        </Panel>
      </div>
