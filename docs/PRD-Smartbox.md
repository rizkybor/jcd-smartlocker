...
| **Touch Screen 7" IPS (1024×600)** | Layar sentuh kiosk untuk interaksi pengguna |
| **Mini PC** | Menjalankan kiosk software (setara "gateway hardware" di §9) |
...
`Internet/LAN` → **Router** (akses jaringan) → **Mini PC/Industrial PC** (kontrol sistem, pemrosesan data, autentikasi pengguna via RFID Reader / QR-Barcode Scanner) → **Power Supply Board** (distribusi daya) → **Main Controller Board** (kontrol semua lock, monitor status, komunikasi ke Mini PC) → **Electric Lock** (buka/tutup tiap kompartemen).
...
|---|---|---|
| **Kiosk app** | React + TypeScript, dibundel dengan Vite, jalan di **browser kiosk-mode** (Chromium `--kiosk`) di atas mini-PC Linux | Prototipe sudah React — tinggal migrasi dari script-tag CDN ke build proper (TS, bundling, code-splitting) tanpa ganti paradigma. Kiosk-mode Chromium paling umum & stabil untuk unattended device. |
...
| **Gateway hardware (di dalam unit)** | Node.js atau Python service di Linux SBC (Mini PC, §8.1), komunikasi serial/RS485 ke Main Controller Board, publish status ke MQTT | Memisahkan "otak kiosk" dari kontrol fisik; lihat §8.2 untuk pemetaan ke arsitektur vendor. |
...
| **Testing end-to-end** | Playwright | Simulasikan alur kiosk penuh (§5.1–§5.2) di viewport kiosk **1024×600 landscape** (ukuran final, §8.1/§12 poin 8 — bukan lagi 800×1280 dari desain awal design system) dan alur dashboard admin. |
...
8. **Terjawab**: ukuran layar kiosk final **dikunci ke 7″ IPS, 1024×600, landscape** (spesifikasi company profile, §8.1) — bukan referensi 8″ 1280×800 portrait yang dipakai `docs/design_reference/`. Konsekuensi: kanvas kiosk & breakpoint di `docs/design_reference/` (§13.1) harus **disesuaikan ulang ke 1024×600 landscape** sebelum kiosk UI dibangun dari komponennya — layout satu-kolom-portrait yang ada sekarang tidak berlaku langsung untuk panel landscape yang lebih pendek secara vertikal. Ini pekerjaan desain lanjutan (re-kalibrasi canvas, grid, dan kemungkinan skala ulang beberapa komponen kiosk seperti `IdleScreen`/`QRScreen`), bukan lagi soal menunggu keputusan vendor.
...
- **Kanvas kiosk acuan — SUDAH DIKALIBRASI ULANG (SMB-801)**: `docs/design_reference/` sekarang memakai token `--sl-kiosk-w`/`--sl-kiosk-h` = **1024×600px, landscape-only** (~170ppi, gutter 24px, content-max 976px), sesuai hardware final (§8.1, §12 poin 8). Referensi lama (8″ tablet, 800×1280 portrait, ~189ppi) sudah diganti di token, guideline (`guidelines/kiosk-canvas.card.html`, `type-kiosk-scale.card.html`), dan `readme.md` design system — bukan lagi asumsi awal yang perlu dikerjakan ulang. Implikasi desain: tinggi (600px) sekarang jadi sumbu yang paling sempit (bukan lebar), jadi setiap layar kiosk harus muat step bar + judul + satu blok konten + baris CTA tanpa scroll — pola split 60/40 (konten kiri, keypad/QR kanan) lebih diutamakan daripada susun vertikal.
...
- Ukuran layar kiosk **sudah diputuskan dan kanvas design system sudah dikalibrasi ulang** (7″ 1024×600 landscape, §12 poin 8, SMB-801) — tidak ada lagi pekerjaan tertunda di titik ini, kiosk UI (Epic 3/4, `client/kiosk`) bisa mulai dibangun langsung di atas canvas final.
...
| **Kiosk yang terhubung** | Simulator/browser biasa (bukan hardware fisik) | Idealnya 1 unit fisik "kiosk staging" khusus (kalau ada) — kalau belum ada unit fisik (§12 poin 1), tetap browser simulator dengan viewport 1024×600 (§8.1, §13.1) | Unit fisik sungguhan di lokasi mitra |
