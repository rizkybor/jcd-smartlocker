...
⚠️ **Substitution flagged.** No icon set, icon font or SVG sprite was provided — the only asset supplied was the logo PNG. Nothing here was hand-drawn as a stand-in for a real brand icon set.

- **Recommended set: [Lucide](https://lucide.dev)** — 24 px grid, 2 px stroke, round caps. It matches Lexend's geometric-humanist construction and stays legible when scaled up for the kiosk. Load from CDN in consuming projects: `https://unpkg.com/lucide@latest/dist/umd/lucide.js` (or `lucide-react`). Nothing in this system hard-depends on it; every component takes an `icon` prop as a `ReactNode`.
- **Sizing:** dashboard 16 px inline / 20 px in the sidebar's 20 px slot / 24 px standalone. **Kiosk icons are never below 48 px**, 64 px inside `xl` buttons. Stroke weight scales with size — use Lucide's 2 px at 24 px and 2.5–3 px above 48 px.
- **Colour:** icons inherit text colour. A coloured icon means a status and must match the locked status colour exactly.
...
- **Unicode glyphs** are used in this system's own demo cards only (`▦`, `☷`, `⚠`, `⌫`, `✓`) as neutral placeholders so the cards have no CDN dependency. In real product work, replace them with Lucide icons — except `⌫` on the numpad backspace and `✓` for completed steps, which are legitimate as typographic glyphs.
...
- `assets/` — logo lockups (see table above)
- `components/kiosk/` · `components/dashboard/` · `components/motion/` — components, each with `.d.ts`, `.prompt.md`, and one `@dsCard` demo HTML per directory
